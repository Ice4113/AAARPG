...
var text_in_progess : bool = false
var waiting_for_choices : bool =false
...
@onready var potrait_sprite: DialogPotratit = $DialogUI/PotraitSprite
...
func _unhandled_input(event: InputEvent) -> void:
	...
	if(
		...
		if text_in_progess == true:
			content.visible_characters = text_length
			timer.stop()
			text_in_progess = false
			show_dialog_button_indicator(true)
			return 
		elif waiting_for_choices == true:
			return 
		dialog_item_index += 1
...
func show_dialog_button_indicator(_is_visble : bool) -> void:
	dialog_progress_indicator.visible = _is_visble
