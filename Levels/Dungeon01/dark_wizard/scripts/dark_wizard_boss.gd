...
const ENEGRY_EXPLOSION_SCENE : PackedScene = preload("res://Levels/Dungeon01/dark_wizard/energy_explosion.tscn")
const ENEGRY_BALL_SCENE : PackedScene = preload("res://Levels/Dungeon01/dark_wizard/energy_orb.tscn")
...
@onready var persitent_data_handler: PersitentDataHandler = $PersitentDataHandler
...
@onready var door_block: TileMapLayer = $"../Door_Block"



func _ready() -> void:
	persitent_data_handler.get_value()
	if persitent_data_handler.value == true:
...
func shoort_orb() -> void:
	var eb :Node2D = ENEGRY_BALL_SCENE.instantiate()
...
func defeat() -> void:
	...
	persitent_data_handler.set_value()
...
func explosion(_p : Vector2 = Vector2.ZERO) -> void:
	var e : Node2D = ENEGRY_EXPLOSION_SCENE.instantiate()
